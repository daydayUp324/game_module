...
public:
     ...
     * 时间复杂度 : O(nlogn)
     * 使用说明 : 
     * 1. 构造函数传入邻接图 g[i] = {{node, edge_id}}, 其中 edge_id 表示该边在边集合中的索引
     */
    vector<long long> dis; // 最短路
    vector<vector<pair<int, int>>>& g; // g[i] = {{node, edge_id}}, 边 i->node 在边集合中索引为 edge_id
    vector<vector<int>>& es; // 边集合 {a, b, v}, 边 a->b 的距离为 v
    int n; // 顶点数
    void Cal_shortestPath(int start, int INF = 0x3f3f3f3f) { // 从 start 开始跑最短路
        ...
        while (!q.empty()) {
            ...
            vi[u] = true;
            for (auto& next : g[u]) {
                int vv = edges[next.second][2]; // 改边的权值
                // DO Something
    ...
    Dijkstra(vector<vector<pair<int, int>>>& g_, vector<vector<int>>& edges
    ) : g(g_), es(edges) {
        n = g.size();
    }
};
