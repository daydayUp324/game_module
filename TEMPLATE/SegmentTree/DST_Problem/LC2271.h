...
int bulid_subTree(int l_,int r_,bool lazy_ = false,int val_ = 0) { // 分配一个子树
...
void push_down(Node& root) { // 从当前节点往下传递信息
    ...
    if(!root.lson) {
        ...
        tn[root.lson].sum = root.val * ((root.l + root.r) / 2 - root.l + 1);
    ...
    if(!root.rson) {
        ...
        tn[root.rson].sum = root.val * (root.r - (root.l + root.r) / 2);
...
void push_up(Node& root) { // 两棵子树往上传递信息
    ...
    else root.sum += ((root.l + root.r) / 2 - root.l + 1) * root.val;
    if(root.rson) root.sum += tn[root.rson].sum;
    else root.sum += (root.r - (root.l + root.r) / 2) * root.val;
}
...
int dst_query(int root,const int tl,const int tr) { // 查询 [tl,tr] 区间和
    Node& temp = tn[root];
...
class Solution {
public:
    int maximumWhiteTiles(vector<vector<int>>& s, int cl) {
        int r = 1e9 + 7;
        int root = bulid_subTree(1,1e9);
        for(auto& i : s) {
            dst_update(root,i[0],i[1],1);
        }
        int res = 0;
        for(auto& i : s) {
            res = max(res,dst_query(root,i[0],min(r,cl + i[0] - 1)));
        }
        return res;
    }
};