...
int bulid_subTree(int l_,int r_,bool lazy_ = false,LL val_ = 0) { // 分配一个子树
    ...
    tn[cur_idx].val = val_;
    tn[cur_idx].sum = (LL) (r_ - l_ + 1) * val_;
    return cur_idx ++;
}
...
LL query(int root,const int tl,const int tr) { // 查询 [tl,tr] 区间和
    Node& temp = tn[root];
    ...
    LL res = 0;
    int mid = (temp.l + temp.r) / 2;
    if(temp.lson) res += query(temp.lson,tl,tr);
    if(temp.rson) res += query(temp.rson,tl,tr);
    return res;
}
class Solution {
public:
    vector<int> fullBloomFlowers(vector<vector<int>>& flowers, vector<int>& persons) {
        ...
        vector<int> res;
        for(auto& i : persons) res.push_back(query(root,i,i));
        return res;
    }
};