...
public:
    ...
    void update(int l, int r, int v = 1) { // 对区间 [l, r] 中的数全部赋值 v（默认为 1）
    ...
    MT dst_query(int root, const int tl, const int tr) { // 查询 [tl,tr] 区间和
        Node& temp = tn[root];
    ...
    DST_ASSIGN_SUM(int l_, int r_) : ori_l(l_), ori_r(r_), cur_idx(1) {
...
public:
    int maximumWhiteTiles(vector<vector<int>>& tiles, int carpetLen) {
        DST_ASSIGN_SUM<int> h(0, 2e9 + 7);
        for (auto& i : tiles) {
            h.update(i[0], i[1]);
        }
        int res = 0;
        for (auto& i : tiles) {
            res = max(res, h.query(i[0], i[0] + carpetLen - 1));
        }
        return res;
    }
};