...
public:
    ...
    DST_ASSIGN_M(int l_, int r_, bool lazy_ = true) : ori_l(l_), ori_r(r_), cur_idx(1) {
        root_id = this->build_subTree(l_, r_, lazy_); // 创建初始节点
    }
    ~DST_ASSIGN_M() {}
};
class Solution {
public:
    vector<int> fallingSquares(vector<vector<int>>& positions) {
        int lmin = 0, rmax = 1e9 + 7;
        DST_ASSIGN_M<int> h(lmin, rmax);
        vector<int> res;
        for (auto& i : positions) {
            int l = i[0], r = i[0] + i[1] - 1;
            int t = h.query(l, r);
            h.update(l, r, t + i[1]);
            res.push_back(h.query(lmin, rmax));
        }
        return res;
    }
};