 ...
 */
#define INF INT_MAX // 标记最值（如果维护的是最小值 INF = INT_MAX，否则 INF = INT_MIN）
#define OP min // 设置操作
...
public:
    ...
    ST_ADD_MIN(int l_ = 0, int r_ = 0) : seq_l(l_), seq_r(r_), n(r_ - l_ + 1) {
        // arr[i] 默认为 0
        arr = new MT[n + 1];
        for (int i = 0; i <= n; ++i) {
            arr[i] = 0;
    ...
    ST_ADD_MIN(const vector<int>& nums) {
        n = nums.size();
        seq_l = 0, seq_r = n - 1;
        arr = new MT[n + 1];
        tree = new MT[4 * n + 1];
...
int main() {
    ...
    ST_ADD_MIN<int> st(nums);
    int l, r;
    for (int i = 0; i < m; ++i) {
        scanf("%d %d", &l, &r);
        printf("%d ", st.query(l - 1, r - 1));
    }
    printf("\n");
    return 0;
}