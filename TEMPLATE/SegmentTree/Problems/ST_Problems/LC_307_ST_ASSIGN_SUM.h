 ...
 * @author : daydayUppp
 * 
 * 区间赋值的线段树 (维护的区间性质为 sum)
 * 
 * (l, r, v) : 将 [l:r] 赋值 v（nums[i] = v, l <= i <= r）
 * query(l, r) : 求 [l:r] 区间和
 * 
 * 若一个区间为 (l, r) 节点序号为 id
...
public:
    ...
    void update_node(int node_len, int node_id, int v) { // 对 node_id 全部等于 v
    ...
    void update(int l, int r, int v = 1) { // 区间 (l, r) 全部赋值 v 默认 = 1
    ...
    void help_update(const int tl, const int tr, int cl, int cr, int node_id, int v) {
        ...
        if (cl >= tl && cr <= tr) { // 当前区间被包含
            update_node(cr - cl + 1, node_id, v); // 进行赋值
            lazy[node_id] = v; // 更新懒标记
            return ;
        }
        // 否则往下
        if (lazy[node_id] != INF) { // 若存在懒标记
    ...
    ST_ASSIGN(int l_ = 0, int r_ = 0) : seq_l(l_), seq_r(r_), n(r_ - l_ + 1) {
        // arr[i] 默认为 0
        arr = new MT[n + 1];
        for (int i = 0; i <= n; ++i) {
            arr[i] = 0;
    ...
    ST_ASSIGN(const vector<int>& nums) {
        n = nums.size();
        seq_l = 0, seq_r = n - 1;
        arr = new MT[n + 1];
        tree = new MT[4 * n + 1];
