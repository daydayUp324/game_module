 ...
 */
#define INF INT_MIN // 标记最值（如果维护的是最小值 INF = INT_MAX，否则 INF = INT_MIN）
#define OP max // 设置操作
#define NO_LAZY INT_MAX // 没有懒标记的设置
...
public:
    ...
    void help_update(const int tl, const int tr, int cl, int cr, int node_id, int v) {
        ...
        // 否则往下
        if (lazy[node_id] != NO_LAZY) { // 若存在懒标记
    ...
    ST_ASSIGN_M(int l_ = 0, int r_ = 0) : seq_l(l_), seq_r(r_), n(r_ - l_ + 1) {
        // arr[i] 默认为 0
        arr = new MT[n + 1];
        for (int i = 0; i <= n; ++i) {
            arr[i] = 0;
    ...
    ST_ASSIGN_M(const vector<int>& nums) {
        n = nums.size();
        seq_l = 0, seq_r = n - 1;
        arr = new MT[n + 1];
        tree = new MT[4 * n + 1];
...
public:
    ...
    vector<int> fallingSquares(vector<vector<int>>& positions) {
        ...
        for (auto& i : positions) {
            mp_nums.push_back(i[0]);
            mp_nums.push_back(i[0] + i[1] - 1);
        }
        orderly();
        int n = mp_nums.size();
        ST_ASSIGN_M<int> h(0, n);
        for (auto& i : positions) {
            int a = getId(i[0]), b = getId(i[0] + i[1] - 1);
            int t = h.query(a, b);
            h.update(a, b, t + i[1]);
            res.push_back(h.query(0, n));
        }
        return res;
    }
};